...
import { firstThenApply, powersGenerator, meaningfulLineCount, Empty, volume, surfaceArea, } from "./exercises.js";
describe("The firstThenApply function", () => {
    ...
    it("works", () => {
        ...
        deepEqual(firstThenApply(["", "A", "ABC"], lengthGreaterThan3, lower), undefined);
        deepEqual(firstThenApply(["ABC", "ABCD", "ABCDE"], lengthGreaterThan3, lower), "abcd");
...
describe("The meaningfulLineCount function", async () => {
    ...
    await it("correctly counts lines for the test file", async () => {
        const count = await meaningfulLineCount("../../test-data/test-for-line-count.txt");
        deepEqual(count, 5);
    });
});
describe("The shape functions", () => {
    const sphere = { kind: "Sphere", radius: 5 };
    const box = { kind: "Box", width: 3, length: 4, depth: 5 };
